	...
	namespace ESP {
		...
		void UpdatePanelCache() {
			...
			if (!panelObjects.empty()) {
				printf("[ESP] Found %zu active panels out of %zu total\n", cachedPanels.size(), panelObjects.size());
			}
		}
		...
		void Render() {
			...
			for (auto panel : cachedPanels) {
				...
				// Draw distance
				Unity::Vector3 playerPos = player->GetPosition();
				float dist = worldPos.Distance(playerPos);
